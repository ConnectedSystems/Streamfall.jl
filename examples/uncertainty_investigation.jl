...
if !@isdefined bl_overview
    if !isfile("uncert_baseline_opt.node")
        ...
        # Calibrate node
        res, opt = calibrate!(ihacres_node, climate, obs; metric=func, MaxTime=60)
...
split_func(obs, sim) = Streamfall.naive_split_metric(obs, sim; metric=Streamfall.mean_NmKGE, n_members=365)
uncert_objfunc = (obs, sim) -> 1.0 - weighted_obj_function(obs[burn_in:end], sim[burn_in:end]; metric=split_func)

res, opt = calibrate!(ihacres_node, climate, obs; metric=uncert_objfunc, MaxTime=180)
update_params!(ihacres_node, best_candidate(res)...)
reset!(ihacres_node)
run_node!(ihacres_node, climate)
