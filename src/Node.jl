...
"""
function get_node_id(mg, gauge_id::String)
    v = collect(filter_vertices(mg, :name, gauge_id))
...
function get_gauge(mg, gauge_id::String)::Tuple
    ...
    return v_id, get_prop(mg, v_id, :node)
end


get_node(mg, v_id) = get_prop(mg, v_id, :node)
get_node_name(mg, v_id) = get_prop(mg, v_id, :name)
